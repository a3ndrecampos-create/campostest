...
var CreateGame = function() {
    ...
    // OTIMIZAÇÃO v3: não existe mais nenhuma tela/menu. Quando uma partida
    // termina (o que normalmente não acontece em modo CPU vs CPU, que roda
    // infinitamente, mas pode ocorrer em casos extremos), liberamos os
    // recursos da partida e já iniciamos uma nova luta CPU vs CPU aleatória.
    Game.prototype.startNextCpuBattle = function () {
        this.resetGameData();
        managed_ = null;
        if (typeof window.StartRandomCpuBattle === "function") {
            window.StartRandomCpuBattle();
        }
    };
    ...
    Game.prototype.runGameLoop = function (start) {
        ...
        if (match_.mustQuit()) {
            // OTIMIZAÇÃO: não existe mais char-select (modo história) nem
            // insert-coin (modo arcade) — a única coisa que existe é CPU vs
            // CPU, então qualquer fim de partida só inicia outra automaticamente.
            this.startNextCpuBattle();
    ...
    // OTIMIZAÇÃO: runInsertCoinScreenLoop e runCharSelectLoop foram removidas.
    // Eram usadas apenas pelas telas "Insert Coin" (modo arcade/atrações) e
    // "Char Select" (modo história), que não existem mais nesta versão —
    // o único modo de jogo é o Quick Match, controlado pelo modal de configuração.
