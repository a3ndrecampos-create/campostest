...
// OTIMIZAÇÃO: as telas de Char Select e Insert Coin não são mais usadas
// (o único modo de jogo é o Quick Match), então os loops abaixo foram removidos.
...
// OTIMIZAÇÃO: StartCharacterSelection, StartInsertCoin e ResetGame foram
// removidas — elas pertenciam ao fluxo de modo história/arcade, que não
// existe mais. Para reiniciar, basta usar o próprio menu de Quick Match.
...
//char-select.js continua sendo usado internamente como "fábrica" de jogadores
//(Game.startMatch depende dele), então seu áudio continua sendo pré-carregado.
//insert-coin.js foi removido — não é mais usado em lugar nenhum do fluxo do jogo.
...
// OTIMIZAÇÃO v3: o modo Quick Match (tela de escolher times/estágio) foi
// removido. Agora é só um botão: ele já sorteia 2 dos 5 personagens
// implementados no jogo (Ryu, Ken, Sagat, M.Bison, Akuma) e um estágio, e
// inicia uma luta 100% CPU vs CPU. Quando a luta termina, outra começa
// automaticamente (ver Game.prototype.startNextCpuBattle em game.js).
var CPU_BATTLE_CHARACTERS = [CHARACTERS.RYU, CHARACTERS.KEN, CHARACTERS.SAGAT, CHARACTERS.MBISON, CHARACTERS.AKUMA];
var CPU_BATTLE_STAGES = ["guy", "ken", "ryu", "sodom", "akuma", "sagat", "chunli", "dramatic_battle"];

function pickTwoDistinctCharacters() {
    var i = getRand(CPU_BATTLE_CHARACTERS.length);
    var j = i;
    while (j === i) {
        j = getRand(CPU_BATTLE_CHARACTERS.length);
    }
    return [CPU_BATTLE_CHARACTERS[i], CPU_BATTLE_CHARACTERS[j]];
}

function StartRandomCpuBattle() {
    var chars = pickTwoDistinctCharacters();
    var stage = CPU_BATTLE_STAGES[getRand(CPU_BATTLE_STAGES.length)];
    debug_.startMatch([{A: chars[0], C: true}], [{A: chars[1], C: true}], stage);
}
